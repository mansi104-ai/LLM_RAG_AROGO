...
with st.sidebar:
    ...
    uploaded_files = st.file_uploader(
        "Choose up to 3 PDF textbooks", type="pdf", accept_multiple_files=True
    )
    ...
    for file in uploaded_files[:3]:  # Limit to 3 textbooks
        if file.name not in st.session_state.textbooks:
            with st.spinner(f"Processing {file.name}..."):
                ...
                faiss_index, indexed_texts = create_faiss_index(
                    [node.content for node in index.children]
                )
...
if st.session_state.textbooks:
    ...
    with col1:
        selected_textbook = st.selectbox(
            "Select a textbook:", list(st.session_state.textbooks.keys())
        )

    with col2:
        query = st.text_input(
            "Enter your question:", placeholder="What would you like to know?"
        )
        ...
        with st.spinner("🔍 Searching for answers..."):
            ...
            with st.expander("📖 Relevant Context", expanded=False):
                for i, text in enumerate(relevant_texts, 1):
                    ...
                    st.write(
                        text[:300] + "..."
                    )  # Show first 300 characters of each relevant text
                    st.markdown("---")
...
# Footer
st.markdown("---")
st.markdown("© 2025 (Arsana Labs)[https://arsanalabs.com]")
